...
def calibration_figure_1panel(all_sources,title,fig,combined=None,
                              metric_x=1,metric_ref=0,
                              offset_source=0,scatter_x_source=0,
                              num=None,trim_time=False,
                              lowpass=True,
    ...
    """
    SM(5/21/2021)
    Similar to the 3 figure panel put only plot a figure of the time-serie
    as well as the comparison metrics at the bottom. Return a figure. 
    """
    N=np.arange(len(all_sources))
    if metric_ref<0:
        metric_ref=N[metric_ref]
    ...
    gs = gridspec.GridSpec(5, 3)
    #fig=plt.figure(figsize=(9,5),num=num)
    plt.tight_layout()
    ts_ax = fig.add_subplot(gs[0:4, :])
    ...
    if 1: # Tidal time scale plot:
        ...
        for src_i,src in enumerate(all_sources):
            ...
            ax.plot(src.time.values[slc],src.values[slc]-offsets[src_i],
                    ...
                    **styles[src_i])
        ax.legend(fontsize=8,loc='upper left')
        ax.set_title('Station ='+ str(title), pad=-20)
...
def calibration_axe_1panel(all_sources,title,axe,combined=None,
                              metric_x=1,metric_ref=0,
                              offset_source=0,scatter_x_source=0,
                              num=None,trim_time=False,
                              lowpass=True,
    ...
    """
    SM(5/21/2021)
    Identical as the 1 pannel but take in the ax of a subplot figure so that 
    you can insert it in a loop function to create a subplot with several stations.
    axe should be in the form of a ax[i]. Put the comparison metric as text at the bottom of
    the figure. 
    ...
    if 1: # Tidal time scale plot:
        ...
        for src_i,src in enumerate(all_sources):
            ...
            # clips to the shorter of the two.
            slc=slice(None,min(src.time.shape[0],src.values.shape[0]))
            axe.plot(src.time.values[slc],src.values[slc]-offsets[src_i],
                    label=labels[src_i],
                    **styles[src_i])
        axe.legend(fontsize=8,loc='upper right')
        axe.set_title('Station ='+ str(title))
    ...
    if metric_x is not None:
        ...
        x=0.05
        y=0.25
        axe.text(x,y,tbl,va='top',transform=axe.transAxes,
                family='monospace',fontsize=fontsize,zorder=3)

    fix_date_labels(axe,4)
    
    # zoom to common period
    axe.axis(xmin=combined.time.values[0],
               xmax=combined.time.values[-1])

    
    axe.patch.set_visible(0)
